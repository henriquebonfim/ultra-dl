...
class FileManager:
    ...
    def register_file(
        self, file_path: str, job_id: str, filename: str, ttl_minutes: int = 10
    ) -> DownloadedFile:
        """
    ...
    def delete_file_by_job_id(self, job_id: str) -> bool:
        """
        Delete file associated with a job.

        Used during job cleanup to ensure file is removed when job is archived.
        ...
        # Delete physical file first (Requirement 2.3)
        try:
            self.storage_repo.delete(file.file_path)
        except Exception:
            # Continue even if physical deletion fails
            # Metadata deletion will still proceed
            pass

        # Delete metadata
        return self.file_repo.delete(file.token)

    def cleanup_expired_files(self) -> int:
        """
        Proactively clean up expired files.

        Deletes both physical files and metadata for all expired files.
        This prevents orphaned files from consuming resources.

        Physical files are deleted before metadata (Requirement 2.3) to ensure
        storage is freed even if metadata deletion fails.

        Returns:
            Number of files cleaned up

        Note:
            Handles individual failures gracefully (Requirement 2.1, 2.2).
            Continues processing remaining files even if one fails.
            Failures are not logged here as domain services should not have
            infrastructure dependencies. The application layer should handle
        ...
        for file in expired_files:
            try:
                # Delete physical file FIRST (Requirement 2.3)
                # This ensures storage is freed even if metadata deletion fails
                try:
                    self.storage_repo.delete(file.file_path)
                except Exception:
                    # Continue even if physical deletion fails
                ...
                # Delete metadata AFTER physical file
                self.file_repo.delete(file.token)

                count += 1
            except Exception:
                # Gracefully handle individual failures (Requirement 2.1, 2.2)
