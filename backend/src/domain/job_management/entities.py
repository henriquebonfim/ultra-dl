...
class JobArchive:
    """
    Archived job metadata for audit and logging purposes.
    
    Preserves essential job information after cleanup without
    maintaining the full job lifecycle state machine.
    
    This entity is immutable once created and represents a snapshot
    of a job at the time of archival.
    """
    
    job_id: str
    url: str
    format_id: str
    status: str  # Final status as string (completed/failed)
    created_at: datetime
    ...
    @classmethod
    def from_job(cls, job: DownloadJob) -> 'JobArchive':
        """
        Create archive from active job.
        
        Args:
            job: DownloadJob to archive
            
        Returns:
            JobArchive instance with job metadata
            
        Raises:
            ValueError: If job is not in terminal state
        """
        if not job.is_terminal():
            raise ValueError(
                f"Cannot archive job in {job.status.value} state. "
                "Job must be in terminal state (completed or failed)."
            )
        
        return cls(
            job_id=job.job_id,
            url=job.url,
            format_id=str(job.format_id),
            status=job.status.value,
            created_at=job.created_at,
            completed_at=job.updated_at if job.is_terminal() else None,
            archived_at=datetime.utcnow(),
            error_message=job.error_message,
            error_category=job.error_category,
            download_token=str(job.download_token) if job.download_token else None,
        )
    
    def to_dict(self) -> dict:
        """
        Serialize archive to dictionary for persistence.
    ...
    @classmethod
    def from_dict(cls, data: dict) -> 'JobArchive':
        """
        Deserialize archive from dictionary.
        
        Args:
            data: Dictionary containing archive data
