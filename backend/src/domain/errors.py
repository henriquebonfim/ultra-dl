...
ERROR_MESSAGES: Dict[ErrorCategory, Dict[str, str]] = {
    ...
    ErrorCategory.AGE_RESTRICTED: {
        ...
        "message": "This video is age-restricted and requires confirmation.",
        "action": "This content cannot be downloaded automatically due to YouTube restrictions.",
    },
}
...
class ApplicationError(Exception):
    ...
    Note: Error logging is the responsibility of the API layer when handling
    these exceptions. Domain events (published by domain entities) are logged
    by event handlers in the infrastructure layer, which is a separate concern
    from API error logging.
    """
...
def create_error_response(
    ...
    Note: This function creates the error response structure but does NOT
    perform logging. Logging is the caller's responsibility (typically the
    API layer that catches and handles these exceptions).
