...
class VideoMetadataExtractor(IVideoMetadataExtractor):
    ...
    def extract_metadata(self, url: YouTubeUrl) -> VideoMetadata:
        ...
        try:
            ...
            raise MetadataExtractionError(
                f"Failed to extract metadata: {str(e)}", original_error=e
            )
    ...
    def extract_formats(self, url: YouTubeUrl) -> List[VideoFormat]:
        ...
        try:
            ...
            raise MetadataExtractionError(
                f"Failed to extract formats: {str(e)}", original_error=e
            )
        except Exception as e:
            raise MetadataExtractionError(
                f"Unexpected error during format extraction: {str(e)}", original_error=e
            )
    ...
    def _determine_resolution(
        self, fmt: Dict[str, Any], height: Optional[int], width: Optional[int]
    ) -> str:
        """
