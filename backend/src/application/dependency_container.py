...
This is the SINGLE dependency injection pattern used throughout the application.
All services should be registered and resolved through this container to maintain
proper layer separation and testability.

USAGE PATTERN:
--------------
1. Services are registered in app_factory.py during application initialization
2. Services are resolved via container.resolve() in tasks and API endpoints
3. Never instantiate services directly - always use the container

LAYER SEPARATION:
-----------------
- Domain Layer: Pure business logic, no DI needed (uses interfaces)
- Application Layer: Services registered as singletons in container
- Infrastructure Layer: Adapters registered as singletons in container
- API/Tasks Layer: Resolve services from container via current_app.container

TESTING:
--------
Use container.override() to inject test doubles/mocks for testing.
Use container.clear_overrides() to clean up after tests.
...
class DependencyContainer:
    ...
    This is the ONLY DI pattern used in the application. No ServiceLocator or
    other patterns should be used. All service instantiation goes through this container.

    REGISTRATION PATTERNS:
    ----------------------
    - Singleton: Single shared instance (use for stateless services)
    - Transient: New instance per resolution (rarely needed)
    - Override: Test doubles for testing (cleared after tests)

    THREAD SAFETY:
    --------------
    All operations are thread-safe for concurrent access in multi-threaded
    environments (Flask + Celery workers).

    EXAMPLE USAGE:
    --------------
    # Registration (in app_factory.py):
    container = DependencyContainer()
    container.register_singleton(DownloadService, download_service)

    # Resolution (in API/tasks):
    from flask import current_app
    download_service = current_app.container.resolve(DownloadService)

    # Testing (in tests):
    container.override(DownloadService, mock_download_service)
    # ... run tests ...
    ...
    def get_metadata_extractor(self):
        ...
        if self._metadata_extractor is None:
            from src.infrastructure.video_metadata_extractor import (
    ...
    def get_storage_repository(self):
        ...
        Returns:
            IFileStorageRepository implementation for local storage
    ...
    def setup_event_handlers(
        self, event_publisher, event_handler_classes: List[Type] = None
    ) -> None:
        """
        ...
        """
        from src.infrastructure.event_handlers.logging_handler import (
            LoggingEventHandler,
        )
        ...
        for handler_class in event_handler_classes:
            try:
                ...
                logger.error(
                    f"Failed to register event handler {handler_class.__name__}: {e}"
                )
