...
from src.domain.job_management import (
    DownloadJob,
    JobManager,
    JobNotFoundError,
    JobProgress,
...
class JobService:
    """
    Application service for job lifecycle CRUD operations.

    Handles job creation, retrieval, status updates, and deletion operations.
    This service provides the interface for job management used by:
    - API layer endpoints (REST API for job operations)
    - WebSocket event handlers (real-time status updates)

    JobService focuses on individual job lifecycle management and does NOT
    orchestrate the full download workflow. For complete download orchestration
    including video processing and file storage, see DownloadService.

    Responsibilities:
    - Create new download jobs
    - Query job status and metadata
    - Update job progress and state transitions
    ...
    def cleanup_expired_jobs(self, expiration_hours: int = 1) -> int:
        """
        Clean up expired jobs with archival and file deletion.

        Coordinates the cleanup process by:
        1. Passing file_manager to job_manager for file deletion coordination
        2. Job manager handles archival, file deletion, and job deletion
        3. Proper error handling and logging at application layer
        ...
        Note:
            Individual cleanup failures are handled gracefully by the job_manager.
            This method logs the overall result and any errors encountered.
        """
        try:
            expiration_time = timedelta(hours=expiration_hours)
            count = self.job_manager.cleanup_expired_jobs(
                expiration_time, 
