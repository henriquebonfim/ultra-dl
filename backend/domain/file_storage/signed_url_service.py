...
Can be extended to support GCS signed URLs when GCS integration is added.
"""
...
class SignedUrlService:
    ...
    def create_gcs_signed_url(
        self, gcs_repository, blob_name: str, ttl_minutes: int = 10
    ) -> str:
        """
        Generate a GCS signed URL using the GCS repository.

        Args:
            gcs_repository: GCSRepository instance
            blob_name: GCS blob/object name
            ttl_minutes: Time to live in minutes
        ...
        Raises:
            Exception: If GCS signed URL generation fails
        """
        return gcs_repository.generate_signed_url(blob_name, ttl_minutes)
