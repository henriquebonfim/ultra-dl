...
from domain.errors import MetadataExtractionError, ErrorCategory
...
class VideoService:
    ...
    def __init__(
        ...
        Args:
            video_processor: Optional VideoProcessor instance (created if not provided)
        ...
        """
        # For backward compatibility, create VideoProcessor if not provided
        # In production, this should always be injected
        if video_processor is None:
            from infrastructure.video_metadata_extractor import VideoMetadataExtractor
            video_processor = VideoProcessor(VideoMetadataExtractor())
        
        self.video_processor = video_processor
        self.cache_service = cache_service
        self.event_publisher = event_publisher
    
    def _categorize_extraction_error(self, error: MetadataExtractionError) -> ErrorCategory:
        """
    ...
    def get_video_info(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Successfully fetched {len(formats_list)} formats for video: {metadata_dict['title']}")
            ...
            logger.error(f"Metadata extraction failed for URL {url}: {error_category.value}")
    ...
    def get_formats_only(self, url: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Categorize the error
            error_category = self._categorize_extraction_error(e)
            logger.error(f"Format extraction failed for URL {url}: {error_category.value}")
